...
def load_config(path:Path)->Config:
    ...
    optional={"limits_file","mtu","max_packets_per_tick"}
    ...
    for name,low,high in (("interface_fd",0,1<<20),("bind_port",0,65535),("peer_port",1,65535),("mtu",576,9000),("max_packets_per_tick",1,256)):
    ...
    return Config(mode,value["core"],value["interface_fd"],value["bind_host"],value["bind_port"],
                  value["peer_host"],value["peer_port"],key,limits,value.get("mtu",1400),value.get("max_packets_per_tick",64))
...
def run(config:Config)->None:
    ...
    endpoint=DatagramEndpoint(config.core,load_key(config.key_file),(config.bind_host,config.bind_port),
                              (config.peer_host,config.peer_port),limits=load_limits(config.limits_file))
    selector=selectors.DefaultSelector(); selector.register(config.interface_fd,selectors.EVENT_READ)
    try:
        while True:
            for _key,_ in selector.select(.05)[:config.max_packets_per_tick]:
                packet=os.read(config.interface_fd,config.mtu+19); validate_packet(config.mode,packet,config.mtu); endpoint.send(0,packet)
            for stream,packet in endpoint.poll(0):
                if stream!=0: raise ValueError("virtual adapter received a non-packet stream")
                validate_packet(config.mode,packet,config.mtu); os.write(config.interface_fd,packet)
    finally: endpoint.close(); selector.close()
...
def main()->int:
    ...
    except (OSError,ValueError,PermissionError,TimeoutError) as exc: print(f"virtual adapter: {exc}",file=sys.stderr); return 1
