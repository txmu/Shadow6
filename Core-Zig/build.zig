...
pub fn build(b: *std.Build) void {
    ...
    const exe = b.addExecutable(.{ .name = "shadow6-zig", .root_source_file = b.path("src/main.zig"), .target = target, .optimize = optimize, .link_libc = true });
    exe.pie = true;
    if (target.result.abi == .android) {
        ...
        exe.root_module.addSystemIncludePath(.{ .cwd_relative = b.fmt("{s}/usr/include/{s}", .{ sysroot, triple }) });
    }
    b.installArtifact(exe);
    exe.root_module.addCSourceFile(.{ .file = b.path("src/signals.c"), .flags = &.{ "-Wall", "-Wextra", "-Werror" } });
