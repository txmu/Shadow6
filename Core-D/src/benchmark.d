...
private bool sendPacket(int socket, ref const SessionId session,
                    ref const Key cipherKey, uint sequence,
                    const(ubyte)[] payload) {
    Wire wire; if (!encodePacket(PacketKind.data, session, sequence, d_now(), payload, cipherKey, wire)) return false;
...
int runBenchmark(uint payloadBytes, uint requests) {
    ...
    Key seed, publicKey, cipherKey, reverseKey; Secret signer; SessionId session;
    foreach (i; 0 .. seed.length) seed[i]=cast(ubyte)(i+1);
    if (!random(cipherKey) || !random(reverseKey)) goto done;
    ...
    foreach(sequence;0..requests) {
        ...
        if(!sendPacket(client,session,cipherKey,sequence,reply[0..payloadBytes])) goto done;
        ...
        if(n<1||!decodePacket(wire[0..n],session,d_now(),cipherKey,decoded)||decoded.length!=payloadBytes) goto done;
        ...
        if(!sendPacket(agent,session,reverseKey,sequence,reply[0..payloadBytes])) goto done;
        n=receive(relay,wire,from); if(n<1||d_udp_send(relay,wire.ptr,n,clientPeer.ptr)) goto done;
        n=receive(client,wire,from);
        if(n<1||!decodePacket(wire[0..n],session,d_now(),reverseKey,decoded)||decoded.length!=payloadBytes||d_write(proxy,decoded.payload.ptr,cast(int)decoded.length)||d_read(app,reply.ptr,cast(int)payloadBytes,1)!=cast(int)payloadBytes) goto done;
