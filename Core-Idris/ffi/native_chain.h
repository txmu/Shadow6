...
static int idris_chain_broker(const struct sockaddr_in *bind_sa,
    ...
    for (unsigned int count=0; count<1000000 && forwarded<limit; ++count) {
        time_t now = idris_chain_now();
        if (!now || now-started>=300 || now-activity>=(phase==2?60:5)) break;
        struct pollfd p = {.fd=fd,.events=POLLIN};
