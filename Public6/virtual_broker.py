...
import argparse, asyncio, base64, hashlib, hmac, ipaddress, json, os, stat, struct, sys, time
...
def load_config(path:Path)->Config:
    ...
    if not 1024<=limits["max_frame"]<=16*1024*1024 or not 1<=limits["handshake_seconds"]<=30 or not 5<=limits["idle_seconds"]<=3600: raise ValueError("limit outside safe bounds")
...
class Broker:
    def __init__(self,config:Config):
        self.config=config; self.approved=config.approvals; self.runtime={x:RuntimeTenant(tokens=t.burst_bytes) for x,t in config.tenants.items()}; self.nonces={}
    def admit(self,raw:bytes)->tuple[Tenant,str,tuple[str,int]]:
        ...
        now=int(time.time())
        if type(request["issued"]) is not int or type(request["expires"]) is not int or request["issued"]>now+30 or request["expires"]<now or request["expires"]-request["issued"]>120: raise PermissionError("expired admission")
        ...
        unsigned={k:v for k,v in request.items() if k!="signature"}; Ed25519PublicKey.from_public_bytes(pub).verify(sig,canonical(unsigned))
        ...
        self.nonces={k:v for k,v in self.nonces.items() if v>=now}
    ...
    async def copy(self,reader,writer,tenant:Tenant):
        ...
        while True:
            data=await asyncio.wait_for(reader.read(min(65536,self.config.max_frame)),self.config.idle_seconds)
            if not data: return
            ...
            if len(data)>runtime.tokens: await asyncio.sleep((len(data)-runtime.tokens)/tenant.bytes_per_second); runtime.tokens=0
            else: runtime.tokens-=len(data)
            writer.write(data); await writer.drain()
    async def handle(self,reader,writer):
        tenant=None; upstream_writer=None
        try:
            size=struct.unpack("!I",await asyncio.wait_for(reader.readexactly(4),self.config.handshake_seconds))[0]
            if not 1<=size<=16384: raise ValueError("invalid admission length")
            tenant,_virtual,target=self.admit(await reader.readexactly(size)); runtime=self.runtime[tenant.tenant_id]
            if runtime.active>=tenant.max_connections: raise PermissionError("tenant connection quota reached")
            runtime.active+=1; upstream_reader,upstream_writer=await asyncio.open_connection(*target)
            await asyncio.gather(self.copy(reader,upstream_writer,tenant),self.copy(upstream_reader,writer,tenant))
        except (asyncio.IncompleteReadError,asyncio.TimeoutError,OSError,ValueError,PermissionError): pass
        finally:
            if tenant: self.runtime[tenant.tenant_id].active=max(0,self.runtime[tenant.tenant_id].active-1)
            for item in (upstream_writer,writer):
                if item: item.close()
    async def serve(self):
        server=await asyncio.start_server(self.handle,self.config.listen_host,self.config.listen_port,limit=self.config.max_frame)
